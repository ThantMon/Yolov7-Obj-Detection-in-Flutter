...
class Classifier {
  ...
  static const String MODEL_FILE_NAME = "ssd/ssd_mobilenet.tflite";
  static const String LABEL_FILE_NAME = "assets/ssd/labels.txt";
  ...
  late bool isImageProcessorNew = true;
  ...
  Future<void> loadLabels() async {
    try {
      ...
      print("Labels are successfully loaded");
    } catch (e) {
  ...
  TensorImage getProcessedImage(TensorImage inputImage) {
    ...
    if (isImageProcessorNew) {
      imageProcessor = ImageProcessorBuilder()
          .add(ResizeWithCropOrPadOp(padSize, padSize))
          .add(ResizeOp(INPUT_SIZE, INPUT_SIZE, ResizeMethod.NEAREST_NEIGHBOUR))
          .add(NormalizeOp(0, 1))
          .build();
      isImageProcessorNew = false;
    }
    inputImage = imageProcessor.process(_inputImage);
    return inputImage;
  }
  ...
  List<Recognition> predict(imageLib.Image image) {
    ...
    print("Inference time : $inferenceTimeElapsed ms");
